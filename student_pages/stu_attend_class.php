...
$r = mysqli_query($con,"SELECT subject.subjcode, subject.subjname, login_faculty.tname, login_faculty.meeting_url from login_faculty, subject WHERE login_faculty.id = subject.tid AND subject.subjcode='$sub' AND login_faculty.id='$name'") or die('Error');
...
// <td><a title="View" href="view_subjective_response_f.php?qid='.$qid.'&hid='.$hid.'"><i>View Response</i></a></td>

   // name is tid, $sId=$_SESSION['email']; , curr date, 1

      echo '<tr><td>'.$a1.'</td><td>'.$a2.'</td><td>'.$a3.'</td> <td><a title="View" href="ish_attendance_trial.php?qid='.$name.'&hid='.$a4.'"><i>Join class</i></a></td></tr>';
  }
