...
<div class = "manageuser">
  ...
  $result = mysqli_query($con,"SELECT h.hid, h.quizid, h.stud_ans, h.score, h.indiQues, h.date, q.subjcode, q.name, q.title FROM subjective_history AS h INNER JOIN subjective_quiz AS q on h.quizid=q.subjective_quizid INNER JOIN login_student AS s ON s.email=h.email WHERE h.email='$e' ") or die('Error');
$c=1;
...
while($row = mysqli_fetch_array($result)) {
  ...
  echo '<tr><td>'.$c++.'</td><td>'.$sc.'</td><td>'.$sn.'</td><td>'.$tn.'</td><td>'.$d.'</td><td><a title="View" href="view_subjective_response.php?hid='.$hid.'&qid='.$qid.'"><i>Result</i></a></td></tr> ';
