...
?>
  <?php
  $ttid=$_SESSION["tid"];
  $result = mysqli_query($con,"SELECT time FROM subjective_quiz where subjective_quizid='$ttid'") or die('Error');
...
//global $arr;
  ...
  //   $a= $arr[$x];
    // echo $a;
     $sql= mysqli_query($con,"SELECT * FROM subjective_ques WHERE qid='$p'");
...
//     echo "<script>alert('Response has been recorded.')</script>";
       ...
       echo "<script>window.location.href='stu_subjective_result.php'</script>";
  }
  else
  {
//   echo "<script>alert('Response has been recorded.')</script>";
