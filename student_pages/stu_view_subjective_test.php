...
<div class = "manageuser">
  ...
  <tr>
   ...
   <th>Date</th> 
    ...
    <?php
  $result = mysqli_query($con,"SELECT * FROM subjective_quiz as q inner join coursedetails as c on q.teacher_id=c.tid and q.subjcode=c.subjcode where c.program='$p' and c.branch='$br' and c.semester='$s'  ORDER BY date DESC") or die('Error');
$c=1;
while($row = mysqli_fetch_array($result)) {
  ...
  $tl = $row['time'];
  $in=$row['intro'];
  $eid=$row['subjective_quizid'];
  $da=$row['date'];
  $tk=$row['duplicateTestKey'];
  //$attemp="idk";
  echo '<tr><td>'.$c++.'</td><td>'.$sc.'</td><td>'.$sn.'</td><td>'.$tn.'</td><td>'.$da.'</td><td>'.$tq.'</td><td>'.$cor.'</td><td>'.$tm.'</td><td>'.$tl.' seconds </td>
  <td><a title="Take Test" href="takeSubjectiveTest1.php?tid='.$eid.'&corr='.$cor.'&tq='.$tq.'&tn='.$sn.'&d='.$in.'"><i>Take Test</i></a></td><td><i>'.$tk.'</i></td></tr>';
}
$c=0;
echo '</table></div>';
?>
