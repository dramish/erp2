...
$sql2 = mysqli_query($con,"SELECT avg(h.score), max(h.score) , min(h.score) ,stddev(h.score), variance(h.score), count(*) from subjective_quiz as q INNER JOIN subjective_history as h ON q.subjective_quizid = h.quizid INNER JOIN login_student as s on s.email=h.email WHERE q.teacher_id='$idf' AND h.quizid='$testid' AND s.program='$p' AND s.branch='$b' AND s.semester='$s' ") or die('No Entries');
