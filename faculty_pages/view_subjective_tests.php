...
<div class = "manageuser">
  ...
  <?php
$idf= $_SESSION['idf'];
  $result = mysqli_query($con,"SELECT * FROM subjective_quiz where teacher_id='$idf' ORDER BY date DESC") or die('No Entries');
$c=1;
...
while($row = mysqli_fetch_array($result)) {
  ...
  $tl = $row['time'];
  $in=$row['intro'];
  $qid=$row['subjective_quizid'];
 
  echo '<tr><td>'.$c++.'</td><td>'.$sc.'</td><td>'.$sn.'</td><td>'.$tn.'</td><td>'.$tq.'</td><td>'.$tm.'</td><td>'.$tl.'</td><td>'.$dat.'</td>
  <td><a title="Delete Test" href="delSubjectiveTest.php?tid='.$qid.'&tname='.$tn.'"><i>Remove</i></a></td>
  <td><a title="Initiate Test" href="genTestKey_subjective.php?tid='.$qid.'&tname='.$tn.'"><i>Start</i></a></td>
  <td><a title="Close Window" href="closewin_subjective.php?tid='.$qid.'&tname='.$tn.'"><i>Close</i></a></td>
  <td><a title="Stop Test" href="hard_stop_subjective_test.php?tid='.$qid.'&tname='.$tn.'"><i>Stop</i></a></td>
    <td><a title="View Questions" href="view_subjective_ques.php?tid='.$qid.'&tname='.$tn.'"><i>View Questions</i></a></td></tr>';
}
$c=0;
echo '</table></div></div>';
