...
<div class = "manageuser">
  ...
  //$n="";
  $idf= $_SESSION['idf'];
    $sql1 = mysqli_query($con,"SELECT s.name,s.rollno, h.hid, h.score,h.stud_ans from subjective_quiz as q INNER JOIN subjective_history as h ON q.subjective_quizid = h.quizid INNER JOIN login_student as s on s.email=h.email WHERE q.teacher_id='$idf' AND h.quizid='$testid' AND s.program='$p' AND s.branch='$b' AND s.semester='$s' ORDER BY s.rollno ") or die('No Entries');
     ...
     while($row=mysqli_fetch_array($sql1) ) {
      ...
      echo '<tr><td>'.$sno++.'</td><td>'.$n.'</td><td>'.$r.'</td><td>'.$sc.'</td><td><a title="Click here to view" href="view_subjective_response_f.php?qid='.$testid.'&hid='.$hid.'"><i>View Response</i></a></td></tr>';
