...
<div class="main">
<?php echo '<h1>Result of '.$testname.'</h1>'; ?>
</div>
 <br>
<div class = "manageuser">
  ...
  //$n="";
  $idf= $_SESSION['idf'];
    $sql1 = mysqli_query($con,"SELECT DISTINCT s.program , s.branch , s.semester , q.totalques,q.correct from subjective_quiz as q INNER JOIN subjective_history as h ON q.subjective_quizid = h.quizid INNER JOIN login_student as s on s.email=h.email WHERE q.teacher_id='$idf' AND h.quizid='$testid' ") or die('No Entries');
     ...
     while($row=mysqli_fetch_array($sql1) ) {
      ...
      echo '<tr><td>'.$sno++.'</td><td>'.$p.'</td><td>'.$b.'</td><td>'.$s.'</td><td><a title="Click here to view result." href="batchwiseres_subjective.php?tid='.$testid.'&p='.$p.'&b='.$b.'&s='.$s.' "><i>Result</i></a></td><td><a title="Click here to analyze result." href="analyzeres_subjective.php?tid='.$testid.'&p='.$p.'&b='.$b.'&s='.$s.'&tq='.$tq.'&c='.$corr.'&tn='.$testname.' "><i>Analyze</i></a></td></tr>';
